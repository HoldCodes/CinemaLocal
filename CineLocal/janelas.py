...
class Principal(funcDb):
    ...
    def insere_historico(self):
        ...
        lista = self.cursor.execute(
            """ SELECT reserva.order_id, reserva.nome_cliente, reserva.cpf_cliente, filmes.nome_filme, reserva.secao_id ,reserva.total_preco FROM reserva, filmes, secao
            WHERE secao_id = secao.id AND secao.filmes_id = filmes.id ORDER BY order_id;""")
        for i in lista:
            # insere na treeview
...
class Filme(funcDb):
    ...
    def janela_horarios(self, filme):
        ...
        for i in range(len(horarios_disp)):
            lista_bt.append(Button(frame3, text='Sessão ' + str(horarios_disp[i][1]) + '\n' + horarios_disp[i][0],
                                   ...
                                   command=lambda x=i: (
                                   self.reserva.set_sessao(horarios_disp[x][1]), self.janela_poltrona(),
                                   newWindow.destroy())))
    ...
    def janela_poltrona(self):
        ...
        btn_confirma = Button(frame4, text="Avançar", font=("Arial", 20), fg="White", background="Black", command= self.verificaRN002)
