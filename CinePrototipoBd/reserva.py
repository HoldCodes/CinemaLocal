...
class Reserva(funcDb):
    ...
    def get_sala_poltrona(self):
        self.conecta_bd()
        self.cursor.execute(
            """SELECT poltrona_id FROM secao WHERE id="""+str(self.sessao))
        id = self.cursor.fetchone()
        self.desconecta_bd()
        return id[0]
    def get_valor_unitario(self):
        self.conecta_bd()
        self.cursor.execute("""SELECT filmes_id FROM secao WHERE id="""+str(self.sessao))
...
class sessoes(funcDb):
    ...
    def busca_todas_secoes(self):
        self.conecta_bd()
        self.cursor.execute(
            """SELECT * FROM secao ORDER BY horario_secao;""")
    ...
    def busca_horario_id(self,filme):
        ...
        for i in range(len(self.lista)):
            if self.lista[i][2]==filme:
                self.lista_hr.append((self.lista[i][3],self.lista[i][1],self.lista[i][0]))
