...
class Validadores:
    def validate_entry(self):
        list_entry=[self.nome_entry,self.genero_entry,self.idade_entry,self.duracao_entry,self.entry_dub]
        #campo em branco
        for i in list_entry:
            if not (self.check_empty(i)):
                messagebox.showinfo(title="Dados invalidos", message="Por favor, preencha todos os campos")
                return False
        #idade string
        try:
            int(self.idade_entry.get())
        except ValueError:
            messagebox.showinfo(title="Dados invalidos", message="Por favor, insira uma idade valida")
            return False
        #duracao string
        try:
            int(self.duracao_entry.get())
        except ValueError:
            messagebox.showinfo(title="Dados invalidos", message="Por favor, insira uma idade valida")
            return False
        return True
...
class filmesGui(Validadores):
    ...
    def frameDataBody(self):
        ...
        self.lb_horario = Label(self.frame_data_body, text="Selecione um horario disponivel")
        self.lb_horario.place(relx=0.03, rely=0.55, relwidth=0.25, relheight=0.1)
        self.user_input = StringVar(self.frame_data_body)
        self.categorias = ("14:30", "16:00", "20:00")
        self.user_input.set("14:30")
    ...
    def frameTreeList(self):
        ...
        self.lista_tree.heading("#4", text="Horario da seção")
    ...
    def insertMovieButton(self):
        if(self.validate_entry()):
            self.conecta_bd()
            self.cursor.execute(
                """ INSERT INTO filmes(nome_filme,genero_filme,idade_filme,duracao_filme, horarios_disponiveis, poster_filme, dublado) VALUES(?,?,?,?,?,?,?)""",
    ...
    def selectButton(self):
        ...
        lista = self.cursor.execute(
            """ SELECT id, nome_filme, genero_filme, horarios_disponiveis,dublado  FROM filmes ORDER BY nome_filme ASC;""")
    ...
    def buttonSearch(self):
        ...
        self.cursor.execute("""SELECT id, nome_filme,genero_filme,horarios_disponiveis, dublado
