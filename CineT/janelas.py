...
class Filme(funcDb):
    ...
    class janela_poltrona(funcDb):
        def __init__(self):
            ...
            for i in range(0, 5):
                for j in range(0, 5):
                    # tentei separar em função esse if else mas garbage collector não deixa
                    index = (i * 5) + j + 1
            ...
            # Frame 4
            btn_confirma = Button(frame4, text="Proximo", font=("Arial", 20), fg="White", background="Black",
        ...
        def atualiza_dados_db(self):
            self.cursor.execute("SELECT * FROM poltronas WHERE poltrona_id=1")
            self.dados = self.cursor.fetchone()

        def escolhe_poltrona(self, poltrona):
            # 1 para ocupado, 2 para selecionado e o outro caso é pra selecionar
        ...
        def altera_status_poltrona(self, poltrona, status):
            self.conecta_bd()
            # gambiarra pra selecionar a coluna pela query
            poltronaString = "p" + str(poltrona - 1)
            print(poltronaString)
